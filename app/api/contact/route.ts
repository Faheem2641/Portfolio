...
export async function POST(request: Request) {
  try {
    ...
    // Option 1: Direct Gmail SMTP via Nodemailer (If GMAIL_USER and GMAIL_APP_PASS are configured)
    ...
    if (web3AccessKey) {
      ...
      if (web3res.ok && web3data?.success) {
        ...
        return NextResponse.json({ error: web3data.message }, { status: web3res.status || 400 })
      }
    }
